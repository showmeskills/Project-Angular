...
export class DepositHistoryComponent implements OnInit {
  constructor(
    ...
    //订阅币种类型 自动带入 by lucy
    this.walletHistoryService.designatedDepositeCurrencyHistory$.pipe(untilDestroyed(this)).subscribe(x => {
  ...
  getStatusTemplate(status: string) {
    const statusList = new Map([
      ['Success', { /*text: this.localeService.getValue('comple00'),*/ calss: 'success' }],
      ['Fail', { /*text: this.localeService.getValue('tran_fail'),*/ calss: 'fail' }],
      ['Timeout', { /*text: this.localeService.getValue('order_expire'),*/ calss: 'cancel' }],
