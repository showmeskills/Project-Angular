...
export class CoinflipApi extends BaseApi {
  /**
   * Coinflip游戏计算
   *
   * @param params
  ...
  async checkData(params: CheckDataParams): Promise<any> {
    const url = `${environment.orignal.newRandomUrl}/coinflip/checkRandomNum`;
  ...
  async getNumberId(): Promise<any> {
    const url = `${environment.orignal.newRandomUrl}/coinflip/publicKey`;
  ...
  async getMultiplier(): Promise<any> {
    console.log(1111);
    const url = `${environment.orignal.newRandomUrl}/coinflip/multiplier`;
    console.log(url);
    const response: Response = await firstValueFrom(this.get<Response>(url));
    return response;
  }
