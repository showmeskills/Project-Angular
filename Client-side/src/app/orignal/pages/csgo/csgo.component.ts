...
export class CsgoComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.orignalService.orignalLoginReady$.pipe(untilDestroyed(this), distinctUntilChanged()).subscribe(v => {
      ...
      if (v) {
        ...
        token = 'test-CSGO-' + Math.floor((Math.random() + Math.floor(Math.random() * 9 + 1)) * Math.pow(10, 5));
      } else {
        return;
      }
      if (!token) {
      ...
      this.ws.init(`${environment.orignal.orignalNewWsUrl}/ws/csgo/open?token=${token}`);
    });
    ...
    if (data.code == 0) {
      ...
      switch (data.action) {
        ...
        case 'actionBetApi':
          ...
          this.orignalService.chartMessage$.next({ gameId: 'csgo', amount: data.data.betAmount, type: 'bet' });
          const item = this.allCurrencyBalance.find(
            balanceItem => balanceItem.currency === this.currentCurrencyData?.currency,
          );
          if (item) {
  ...
  getItemBackgroundbg(odds: string): string {
    const odd = Number(odds);
    if (odd > 0 && odd <= 1.9804) {
      return 'assets/orignal/images/csgo/cs1.png';
    } else if (odd > 1.9804 && odd <= 3.9804) {
      return 'assets/orignal/images/csgo/cs2.png';
    } else if (odd > 3.9804 && odd <= 7.9216) {
      return 'assets/orignal/images/csgo/cs3.png';
    } else if (odd > 7.9216 && odd <= 15.8432) {
      return 'assets/orignal/images/csgo/cs4.png';
    } else if (odd > 15.8432 && odd <= 31.6864) {
      return 'assets/orignal/images/csgo/cs5.png';
    } else if (odd > 31.6864 && odd <= 63.3728) {
      return 'assets/orignal/images/csgo/cs6.png';
    } else {
      // 默认情况下返回一个默认的背景颜色 FF23CF
      return 'assets/orignal/images/csgo/cs7.png';
    }
  }
  getItemResultsColor(odds: string): string {
    const odd = Number(odds);
    if (odd > 0 && odd <= 1.9804) {
      return 'rgba(161, 183, 207, 1)';
    } else if (odd > 1.9804 && odd <= 3.9804) {
      return 'rgba(77, 132, 208, 1)';
    } else if (odd > 3.9804 && odd <= 7.9216) {
      return 'rgba(58, 76, 253, 1)';
    } else if (odd > 7.9216 && odd <= 15.8432) {
      return 'rgba(115, 33, 253, 1)';
    } else if (odd > 15.8432 && odd <= 31.6864) {
      return 'rgba(198, 0, 224, 1)';
    } else if (odd > 31.6864 && odd <= 63.3728) {
      return 'rgba(227, 52, 59, 1)';
    } else {
      // 默认情况下返回一个默认的背景颜色 FF23CF
      return 'rgba(223, 158, 45, 1)';
    }
  }
  // clear() {
  //   this.isBet = false;
