...
import { MinesApi } from '../../shared/apis/mines.api';
  ...
  async ngOnInit() {
    ...
    this.orignalService.orignalLoginReady$.pipe(untilDestroyed(this)).subscribe(async (x: boolean | null) => {
      ...
      if (x) {
        ...
        this.ws.init(`${environment.orignal.orignalNewWsUrl}/ws/mines/open?token=${this.orignalService.token}`);
      }
    });
      ...
      switch (data.action) {
        ...
        case 'actionMinesCurrentApi':
          ...
          if (data.data) {
            ...
            this.orignalService.chartMessage$.next({ gameId: 'mines', amount: data.data.lotteryAmount, type: 'bet' });
        ...
        case 'actionMinesBetApi':
          ...
          this.orignalService.chartMessage$.next({ gameId: 'mines', amount: data.data.betAmount, type: 'bet' });
        ...
        case 'actionMinesSettlementApi':
          ...
          this.orignalService.chartMessage$.next({ gameId: 'mines', amount: data.data.winLossAmount, type: 'set' });
  ...
  openItem(data: any) {
    ...
    if (!this.active) {
      ...
      if (isDiamonds) {
        ...
        this.betService.minesBetstate$.next({ betting: false });
        this.orignalService.chartMessage$.next({ gameId: 'mines', amount: -data.betAmount, type: 'set' });
        this.ws.sendMessage({ action: 'actionGameCodePublicKeyApi', data: { userId: this.uid } });
      }
      this.list = [...this.list];
      // 设置赔率选择框
