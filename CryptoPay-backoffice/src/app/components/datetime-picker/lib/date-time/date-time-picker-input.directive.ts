  ...
  implements OnInit, AfterContentInit, OnDestroy, ControlValueAccessor, Validator
{
  ...
  /**
   * Format input dom show value
   */
  @Input()
  formatLabel = 'yyyy-MM-dd hh:mm';
  ...
  set selectMode(mode: SelectMode) {
    if (mode !== 'single' && mode !== 'range' && mode !== 'date-range' && mode !== 'rangeFrom' && mode !== 'rangeTo') {
  ...
  set value(value: T | null) {
    ...
    if (!this.dateTimeAdapter.isEqual(oldDate as T, value as T)) {
  ...
  private minValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | any => {
  ...
  private maxValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | any => {
    if (this.isInSingleMode) {
      const controlValue = this.getValidDate(this.dateTimeAdapter.deserialize(control.value));
  ...
  get owlDateTimeInputAriaOwns(): string {
    return (this.dtPicker.opened && this.dtPicker.id) || '';
  }
  ...
  public handleInputOnHost(event: any): void {
    ...
    } else if (this._selectMode === 'range' || this._selectMode === 'date-range') {
  ...
  public formatNativeInputValue(): void {
    if (this.isInSingleMode) {
      ...
      this.renderer.setProperty(
        this.elmRef.nativeElement,
        'value',
        this._value ? this.formatTime(this._value, this.formatLabel) : ''
      );
    } else if (this.isInRangeMode) {
      if (this._values && this.values.length > 0) {
        ...
        const fromFormatted = from ? this.formatTime(from, this.formatLabel) : '';

        const toFormatted = to ? this.formatTime(to, this.formatLabel) : '';

        if (!fromFormatted && !toFormatted) {
          this.renderer.setProperty(this.elmRef.nativeElement, 'value', null);
        } else {
          if (this._selectMode === 'range' || this._selectMode === 'date-range') {
            this.renderer.setProperty(
              this.elmRef.nativeElement,
              'value',
  ...
  private changeInputInSingleMode(inputValue: string): void {
    ...
    if (this.dtPicker.pickerType === 'timer') {
      value = this.convertTimeStringToDateTimeString(value, this.value as T) as string;
    }
  ...
  private changeInputInRangeFromToMode(inputValue: string): void {
    ...
    if (this.dtPicker.pickerType === 'timer') {
      inputValue = this.convertTimeStringToDateTimeString(inputValue, originalValue) as string;
    }
  ...
  private changeInputInRangeMode(inputValue: string): void {
    ...
    if (this.dtPicker.pickerType === 'timer') {
      fromString = this.convertTimeStringToDateTimeString(fromString, this.values[0]) as string;
      toString = this.convertTimeStringToDateTimeString(toString, this.values[1]) as string;
    }
