...
import { OwlTimerPickerComponent } from './timer-picker.component';
...
@Component({
  ...
  standalone: true,
  imports: [OwlTimerBoxComponent, NgIf, OwlTimerPickerComponent],
})
export class OwlTimerComponent<T> implements OnInit {
  ...
  get timeValue(): string {
    const base = this.hourBoxValue + ':' + this.minuteValue;
    return base + (this.showSecondsTimer ? ':' + this.secondValue : '');
  }
  ...
  /**
   * time picker confirm event
   */
  public onTimeConfirm(time: any): void {
    const [hours, minutes, seconds] = time.split(':');

    let m = this.dateTimeAdapter.setHours(this.pickerMoment, hours);
    m = this.dateTimeAdapter.setMinutes(m, minutes);

    if (this.showSecondsTimer) {
      m = this.dateTimeAdapter.setSeconds(m, seconds);
    }
