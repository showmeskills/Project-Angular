...
import { OwlDateTime, PickerType } from 'src/app/components/datetime-picker/lib/date-time/date-time.class';
import { Observable, Subject } from 'rxjs';
import { owlDateTimePickerAnimations } from 'src/app/components/datetime-picker/lib/date-time/date-time-picker.animations';
import { DOWN_ARROW, LEFT_ARROW, RIGHT_ARROW, SPACE, UP_ARROW } from '@angular/cdk/keycodes';
import { LangService } from 'src/app/shared/components/lang/lang.service';
import { LangPipe } from 'src/app/shared/components/lang/lang.pipe';
import { OwlTimerComponent as OwlTimerComponent_1 } from './timer.component';
import { OwlCalendarComponent as OwlCalendarComponent_1 } from './calendar.component';
...
@Component({
  ...
  standalone: true,
  imports: [A11yModule, NgIf, NgClass, OwlCalendarComponent_1, OwlTimerComponent_1, LangPipe],
})
export class OwlDateTimeContainerComponent<T> implements OnInit, AfterContentInit, AfterViewInit {
  ...
  public timeSelected(time: T): void {
    ...
    if (this.picker.isInRangeMode) {
      const selecteds = [...(this.picker.selecteds as T[])];
  ...
  private setActiveSelectedIndex(index: number): void {
    if (
      (this.picker.selectMode === 'range' || this.picker.selectMode === 'date-range') &&
  ...
  private dateSelectedInSingleMode(date: T): T | null {
    if (this.dateTimeAdapter.isSameDay(date, this.picker.selected as T)) {
      return null;
    }
  ...
  private dateSelectedInRangeMode(date: T): T[] | null {
    let from: any = this.picker.selecteds![0];
    let to: any = this.picker.selecteds![1];
    ...
    if (this.picker.selectMode === 'range' || this.picker.selectMode === 'date-range') {
      if (
