...
} from 'src/app/components/datetime-picker/lib/date-time/calendar-body.component';
...
import { OwlCalendarBodyComponent as OwlCalendarBodyComponent_1 } from './calendar-body.component';

const MONTHS_PER_YEAR = 12;
const MONTHS_PER_ROW = 3;

@Component({
  selector: 'owl-date-time-multi-month-view',
  templateUrl: './calendar-multi-month-view.component.html',
  styleUrls: ['./calendar-multi-month-view.component.scss'],
  host: {
  ...
  standalone: true,
  imports: [OwlCalendarBodyComponent_1],
})
export class OwlMultiMonthViewComponent<T> implements OnInit, AfterContentInit, OnDestroy {
  /**
  ...
  private _pickerMoment!: any | null;
  @Input()
  get pickerMoment() {
   ...
   */
  private getMonthInCurrentYear(date: any | null): number | null {
  ...
  private setSelectedMonths(): void {
    ...
    if (this.isInSingleMode && this.selected) {
      this.selectedMonths[0] = this.getMonthInCurrentYear(this.selected) as number;
    }

    if (this.isInRangeMode && this.selecteds) {
      this.selectedMonths[0] = this.getMonthInCurrentYear(this.selecteds[0]) as number;
      this.selectedMonths[1] = this.getMonthInCurrentYear(this.selecteds[1]) as number;
    }
  }
