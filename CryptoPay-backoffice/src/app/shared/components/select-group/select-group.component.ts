...
export class SelectGroupComponent implements OnInit, OnDestroy, OnChanges {
  @Input() label = 'label'; // 显示选项的label
  @Input() value = 'value'; // 值属性
  @Input() disabled = 'disabled'; // 禁止属性
  @Input() emptyText = '无选项'; // 空展示文字
  @Input() showSelectAll = true; // 是否显示选择所有 当type为checkbox有效
  @Input('data') propList: any[] = []; // 数据列表
  @Input() select: any[] = []; // 选中的value数组 可双向绑定 -> 需要传select 全部
  ...
  @Output() confirm: EventEmitter<any> = new EventEmitter(); // 确定提交事件
