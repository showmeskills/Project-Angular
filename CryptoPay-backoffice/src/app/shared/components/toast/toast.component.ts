...
@Component({
  ...
  standalone: true,
  imports: [NgFor, NgbToast, NgSwitch, NgSwitchCase, AngularSvgIconModule, NgIf, LangPipe],
})
export class ToastComponent implements OnInit, OnDestroy {
  constructor(private appService: AppService, private lang: LangService, private destroyed$: DestroyService) {}

  toasts: Toast[] = [];

  ngOnInit() {
    this.appService.showToastSubject.pipe(takeUntil(this.destroyed$)).subscribe((x: Toast) => this.processToast(x));
  }
