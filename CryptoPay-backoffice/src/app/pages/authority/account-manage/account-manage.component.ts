...
import { UserItemCustom } from 'src/app/shared/interfaces/user-info-list';
...
export class AccountManageComponent implements OnInit {
  ...
  async updateStatus(temp, item) {
    let en = await this.lang.getOne('auManage.sys.enable');
    let dis = await this.lang.getOne('auManage.sys.disable');
   ...
   * @param item
   */
  async onResetPassword(item: UserItemCustom) {
    if (
      (await this.confirmModalService.open({
        msgArgs: { u: item.userName },
        msgLang: 'common.login.resetPasswordTips',
        titleLang: 'common.login.resetPassword',
      }).result) !== true
    )
      return;

    this.appService.isContentLoadingSubject.next(true);
    this.api.resetPassword({ id: item.id, userName: item.userName }).subscribe((res) => {
