...
export class SubheaderService implements OnDestroy {
  ...
  private menuService = inject(DynamicAsideMenuService)
  ...
  updateAfterRouteChanges(pathName) {
    // 因为是注入的Root服务这里不用销毁，menuConfig是BehaviorSubject不用担心监听失败
    this.menuService.menuConfig$.subscribe(() => {
      if (this.lang.supportLang.some(e => pathName.startsWith('/' + e))) {
        pathName = pathName.match(/^\/(.*?)(\/(.*?)$)/)[2] || pathName;
      }
