...
export class AddComponent implements OnInit, OnDestroy {
  ...
  add() {
    ...
    this.createGroupSubscription = this.api.creatgroup(params).subscribe((res) => {
      ...
      // 添加成功的会员列表
      if (res?.successUserNameList?.length) {
        this.appService.showToastSubject.next({
          msgLang: 'auManage.role.yesAc',
          msgChildren: res.successUserNameList,
          duration: 5e3,
      ...
      if (res?.repeatedUserNameList?.length) {
        this.appService.showToastSubject.next({
          msgLang: 'auManage.role.reAc',
          msgChildren: res.repeatedUserNameList,
          duration: 5e3,
          reactivateDuration: 3e3,
          successed: false,
        });
  ...
  async update() {
    ...
    // 添加成功的会员列表
    if (res1?.successUserNameList?.length) {
      this.appService.showToastSubject.next({
        msgLang: 'auManage.role.yesAc',
        msgChildren: res1.successUserNameList,
        duration: 5e3,
        reactivateDuration: 3e3,
        successed: true,
      });
    ...
    if (res1?.repeatedUserNameList?.length) {
      this.appService.showToastSubject.next({
        msgLang: 'auManage.role.reAc',
        msgChildren: res1.repeatedUserNameList,
        duration: 5e3,
        reactivateDuration: 3e3,
        successed: false,
      });
