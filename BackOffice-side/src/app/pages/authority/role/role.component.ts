...
export class RoleComponent implements OnInit, OnDestroy {
  ...
  paginatorPopup: PaginatorState = new PaginatorState(1, 30); // 弹窗分页
  ...
  // 导出
  async onExport() {
    const open = await this.lang.getOne('common.open');
    const close = await this.lang.getOne('common.off');
    const name = await this.lang.getOne('auManage.role.name');
    const group = await this.lang.getOne('auManage.role.beGroup');
    const openStu = await this.lang.getOne('auManage.role.openStu');
    const qx = await this.lang.getOne('auManage.role.qx');
  ...
  getPopupList(resetPage = false) {
    resetPage && (this.paginatorPopup.page = 1);
    const params = {
      RoleId: this.userPopupId,
      page: this.paginatorPopup.page,
      pageSize: this.paginatorPopup.pageSize,
    };
    this.api[this.isLogList ? 'getPageRoleLog' : 'getPageUserRole'](params)
      .pipe(finalize(() => this.loading(false)))
      .subscribe((res) => {
