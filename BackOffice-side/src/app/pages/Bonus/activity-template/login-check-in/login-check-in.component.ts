...
@Component({
  selector: 'app-login-check-in',
  templateUrl: './login-check-in.component.html',
  styleUrls: ['./login-check-in.component.scss'],
...
export class LoginCheckInComponent implements OnInit {
  constructor(
    ...
    // 必须：活动初始化设置
    this.curType = ActivityTypeEnum.VipSignIn;
    this.instance = LoginCheckInstance;
  }

  /** 必须：活动初始化设置 */
  readonly curType: ActivityTypeEnum;
  readonly instance: ActivityRoute;
  ...
  onGoCampaign(item?: ActivityListItem) {
    ...
    this.router.navigate([this.instance.getPathStep1(item?.activityId, item?.tmpCode)], {
  ...
  onViewCampaign(item: ActivityListItem) {
    ...
    this.router.navigate([this.instance.getViewPathStep1(item.activityId, item.tmpCode)], {
  ...
  async onExport() {
    ...
    const totalSignedInUsers = await this.lang.getOne('member.activity.sencli15.totalSignedInUsers'); // 总签到用户数
    const totalCheckIns = await this.lang.getOne('member.activity.sencli15.totalCheckIns'); // 总签到次数
    ...
    JSONToExcelDownload(
      list.map((e) => ({
        ...
        [totalSignedInUsers]: e.tmpSendUidNum,
        [totalCheckIns]: e.tmpSendNum,
        [actStatus]: status[e.configState] || '-',
      })),
      'vip-sign-in-list ' + Date.now()
    );
  }
}
