...
@Component({
  selector: 'extra-reward',
  templateUrl: './extra-reward.component.html',
  styleUrls: ['./extra-reward.component.scss'],
...
export class ExtraRewardComponent implements OnInit {
  constructor(
  ...
  onAdd(item?: ActivityListItem) {
    ...
    this.router.navigate([extraRewardInstance.getPathStep1(item?.activityId, item?.tmpCode)], {
  ...
  onView(item: ActivityListItem) {
    ...
    this.router.navigate([extraRewardInstance.getViewPathStep1(item.activityId, item.tmpCode)], {
  ...
  async onExport() {
    ...
    JSONToExcelDownload(exportList, 'eWalletDepBonus-list ' + Date.now());
  }
