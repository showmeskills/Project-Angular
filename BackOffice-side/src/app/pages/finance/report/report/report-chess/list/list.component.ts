...
import { ReportChessListDetailComponent } from './detail/detail.component';

@Component({
  selector: 'report-chess-list',
...
export class ReportChessListComponent implements OnInit {
  constructor(
  ...
  loadData$(resetPage = false, sendData?: Partial<any>) {
    ...
    return this.wagerApi.getChessList(parmas);
  }

  /** 查看详情 */
  async onView(item: any) {
    const res = await this.reportDetail.openDetail(
      this.wagerApi.getChessDetail(item.orderNumber),
      ReportChessListDetailComponent
    );
    // 组件内部会自行流完成，不用取消考虑订阅
  ...
  async exportExcel(list: any[]) {
    ...
    JSONToExcelDownload(exportList, 'report-chess-list ' + Date.now());
  }
}
