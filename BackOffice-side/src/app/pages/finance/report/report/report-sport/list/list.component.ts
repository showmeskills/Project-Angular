...
import { ReportSportListDetailComponent } from './detail/detail.component';
import { ReportSportItem } from 'src/app/shared/interfaces/wager';

@Component({
  selector: 'report-sport-list',
...
export class ReportSportListComponent implements OnInit {
  constructor(
  ...
  loadData$(resetPage = false, sendData?: Partial<any>) {
    ...
    return this.wagerApi.getSportList(parmas);
  }

  /** 查看详情 */
  async onView(item: ReportSportItem) {
    const res = await this.reportDetail.openDetail(
      this.wagerApi.getSportDetail(item.orderNumber),
      ReportSportListDetailComponent
    );
    // 组件内部会自行流完成，不用取消考虑订阅
   ...
   * @param list
   */
  async exportExcel(list: ReportSportItem[]) {
    ...
    const merchantOdds = await this.lang.getOne('game.detail.single_odd');
    const handicapType = await this.lang.getOne('game.detail.handicapType');
    ...
    const excelList = await Promise.all(
      list.map(async (e) => {
        return {
          ...
          [merchantOdds]: e?.detailOdds || '-',
          [handicapType]: e?.handicap || '',
        };
      })
    );

    JSONToExcelDownload(excelList, 'report-sport-list ' + Date.now());
  }
}
