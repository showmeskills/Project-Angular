...
import { ReportLotteryListDetailComponent } from './detail/detail.component';

@Component({
  selector: 'report-lottery-list',
...
export class ReportLotteryListComponent implements OnInit {
  constructor(
  ...
  loadData$(resetPage = false, sendData?: Partial<any>) {
    ...
    return this.wagerApi.getLotteryList(parmas);
  }

  /** 查看详情 */
  async onView(item: any) {
    const res = await this.reportDetail.openDetail(
      this.wagerApi.getLotteryDetail(item.orderNumber),
  ...
  async exportExcel(list: any[]) {
    ...
    JSONToExcelDownload(exportList, 'report-lottery-list ' + Date.now());
  }
}
