...
import { ReportRealListDetailComponent } from './detail/detail.component';

@Component({
  selector: 'report-real-list',
...
export class ReportRealListComponent implements OnInit {
  constructor(
  ...
  loadData$(resetPage = false, sendData?: Partial<any>) {
    ...
    return this.wagerApi.getLivecasinoList(parmas);
  }

  /** 查看详情 */
  async onView(item: any) {
    const res = await this.reportDetail.openDetail(
      this.wagerApi.getLivecasinoDetail(item.orderNumber),
  ...
  async exportExcel(list: any[]) {
    ...
    JSONToExcelDownload(exportList, 'report-real-list ' + Date.now());
  }
}
