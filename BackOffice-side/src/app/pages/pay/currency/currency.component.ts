...
export class CurrencyComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.router.events
      ...
      .subscribe(() => {
        if (this.router.routeReuseStrategy['curr'] === 'currency/:merchantId/:currency') this.loadData();
      });
  ...
  getList$() {
    return this.api
      .getCurrencyList({
        TenantId: +this.subHeaderService.merchantCurrentId || 0,
        Name: this.data.name,
   ...
   * @param item
   */
  getSystemRate(item: TenantCurrency) {
    return item.systemRate || 0;
   ...
   * @param event
   */
  onDrop(event: CdkDragDrop<TenantCurrency>) {
    ...
    this.api
      .updateSort(
        ...
        this.list.map((e: TenantCurrency) => e.currency)
      )
      .pipe(
