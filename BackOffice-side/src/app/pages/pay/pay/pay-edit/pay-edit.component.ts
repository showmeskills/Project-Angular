...
export class PayEditComponent implements OnInit {
  ...
  data = {
    ...
    fee: 0,
    feeEnabled: true,
    selectLabel: [] as number[],
    isRecommend: false,
    status: false,
    maxAmount: 0,
    minAmount: 0,
    category: PaymentCategoryEnum.Deposit,
    name: '',
    icon: [] as string[],
  };
  ...
  drop(event) {
    ...
    // labelId数组
    const sortIds = this.labelList.map((item) => item.labelId);
  ...
  ngOnInit(): void {
    ...
    zip(
      ...
      if (!result || !result?.list?.[0]) return;
      const res = result?.list?.[0];
      res.fee = String((res.fee * 1e4) / 100 || '') as any;
  ...
  getLabel() {
    const langCode = this.lang.currentLang?.toLowerCase() || 'zh-cn';
    return this.api.getlabellist(this.queryData.merchantId).pipe(
      tap((label) => {
        const result = label.map(({ labelId, sort, item }) => ({
          item,
          labelId,
          sort,
          name:
            item.find(({ languageCode }) => languageCode === langCode)?.name ||
            item.find(({ languageCode }) => languageCode === 'zh-cn')?.name,
        }));
