...
@Component({
  ...
  templateUrl: './batch-risk-detail.component.html',
  styleUrls: ['./batch-risk-detail.component.scss'],
})
export class BatchRiskDetailComponent {
  public modal = inject(MatModalRef<BatchRiskDetailComponent, boolean>);
   ...
   * 风控数据
   */
  data?: BatchListItem<BatchListRiskDetail>;
   ...
   * @param item {BatchListItem<BatchListRiskDetail>} 单个审批项
   * @param c {function} 关闭弹窗
   */
  async onSubmit(
    pass: boolean,
    item?: BatchListInfo<BatchListRiskDetail>,
    tpl?: TemplateRef<any> | null,
    c?: (boolean) => void
  ) {
    const getInfoItem = (item: BatchListInfo<BatchListRiskDetail>) => ({
