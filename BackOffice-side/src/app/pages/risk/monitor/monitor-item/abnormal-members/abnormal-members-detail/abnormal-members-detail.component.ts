...
import { AccountDisabledComponent } from 'src/app/pages/member/detail/account-disabled/account-disabled.component';
...
@Component({
  selector: 'app-abnormal-popup',
  templateUrl: './abnormal-members-detail.component.html',
  styleUrls: ['./abnormal-members-detail.component.scss'],
  standalone: true,
  imports: [
    ModalTitleComponent,
    NgFor,
...
export class AbnormalMembersDetailComponent implements OnInit {
  constructor(
    public modal: MatModalRef<AbnormalMembersDetailComponent>,
  ...
  /** IP/设备指纹 - 分页 */
  deviceIpPaginator: PaginatorState = new PaginatorState();
  ...
  ngOnInit() {
    if (this.tab === 1) this.auditList.push({ name: '待补充资料', lang: 'risk.toBeSupplemented', value: 'Processing' });

    // IP/设备指纹
    this.deviceIpPaginator.pageSize = 10;
    this.getDeviceIpList();

    // 交易概览
    this.paginator.pageSize = 10;
    this.getProviderStatList();
  ...
  getDeviceIpList() {
    ...
    const params = {
      ...
      uid: this.uid,
      pageIndex: this.deviceIpPaginator.page,
      pageSize: this.deviceIpPaginator.pageSize,
    };
    this.api.getmemberfingerprint(params).subscribe((res) => {
      this.loading(false);

      this.deviceIpList = Array.isArray(res?.list) ? res?.list : [];
      this.deviceIpPaginator.total = res?.total || 0;
    });
  }
  ...
  confirm() {
    ...
    if (['Pending', 'Processing'].includes(this.status)) {
      if (this.audit === 'Rejected') {
        const modal = this.matModal.open(AccountDisabledComponent, {
          width: '750px',
        });
        modal.componentInstance['tenantId'] = this.tenantId;
        modal.componentInstance['uid'] = this.uid;
        modal.componentInstance['isAllow'] = false;
        modal.componentInstance['userDetailsInfo'] = this.userDetailsInfo;

        modal.componentInstance.accDisabledSuccess.subscribe(() => {
  ...
  updateAbnormalMemberStatus() {
    ...
    this.monitorApi.updateAbnormalMemberStatus(params).subscribe((res: any) => {
      this.loading(false);
