...
@Component({
  ...
  templateUrl: './operation-review.component.html',
  styleUrls: ['../../monitor.component.scss', './operation-review.component.scss'],
  providers: [DestroyService],
})
export class OperationReviewComponent implements OnInit {
  constructor(
  ...
  loadData$(resetPage = false, sendParams?: Partial<MonitorServiceParams>) {
    ...
    return this.api.getOperationReview(sendData);
  }

  /**
   * 审核弹窗 - 根据不同审核进入流程
   */
  async onDetail(item: OperationReviewItem) {
    ...
    try {
      switch (category) {
        // 早期功能：gogaming没有冲正和部分冲正、人工编辑，这里也无法直接调用goMoney接口，token不一致
        // case OperationReviewCategoryObjEnum.Edit:
        // case OperationReviewCategoryObjEnum.Reversal: {
  ...
  private async exportBind() {
    ...
    // 获取内容
    const getContent = (e: OperationReviewItem) => {
      const contentList = {
        // 调账
        [OperationReviewCategoryObjEnum.AdjustWallet]: adjustment,
        // 重置手机号码
        [OperationReviewCategoryObjEnum.ResetBindMobile]: resetPhone,
        // 活动审核
        [OperationReviewCategoryObjEnum.Activity]:
          e.detail.status === 1
            ? enabledActivity
            : disabledActivity + ': ' + this.activityService.getLangText(e?.detail?.type),

        // 取消注单
        [OperationReviewCategoryObjEnum.CancelWager]: cancelBet + ':' + e.detail?.wagerNumber,
        // 人工编辑
        [OperationReviewCategoryObjEnum.Edit]: manualEdited + ':' + e.detail?.orderNum,
        // 冲正撤单
        [OperationReviewCategoryObjEnum.Reversal]:
          (e?.detail?.receiveAmount ? reversePartOrder : reverseOrder) + ':' + e.detail?.orderNum,
        // 开启玩家
        [OperationReviewCategoryObjEnum.EnablePlayer]: e.detail?.enableInfo,
        // 优惠劵
        [OperationReviewCategoryObjEnum.Coupon]: e.detail?.status === 1 ? couponOn : couponOff,
        // 发放优惠券
        [OperationReviewCategoryObjEnum.SendCoupon]: sendCoupon,
        // 会员基本信息：修改
        [OperationReviewCategoryObjEnum.MemberBasicInfo]: basicModify,
      };

      return contentList[e.category] || '';
    };

    const getName = (e: OperationReviewItem) => {
