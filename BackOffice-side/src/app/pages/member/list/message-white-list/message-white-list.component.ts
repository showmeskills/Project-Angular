...
import { MessageWhiteListAddPopupComponent } from './add-popup/add-popup.component';
import { MatModal } from 'src/app/shared/components/dialogs/modal';
import { ConfirmModalService } from 'src/app/shared/components/dialogs/confirm-modal/confirm-modal.service';
import { MessageWhiteItem } from 'src/app/shared/interfaces/member.interface';
...
@Component({
  ...
  templateUrl: './message-white-list.component.html',
  styleUrls: ['./message-white-list.component.scss'],
...
export class MessageWhiteListComponent implements OnInit {
  constructor(
  ...
  loadData$(sendData?) {
    ...
    return this.api.getMessageWhiteList(parmas);
  }
  ...
  onDeletion(isBatch = false, uid?: string) {
    ...
    this.confirmModalService
      ...
      .result.then(() => {
        ...
        this.appService.isContentLoadingSubject.next(true);
        this.api.deleteMessageWhiteList(parmas).subscribe((res) => {
          this.appService.isContentLoadingSubject.next(false);
  ...
  /** 新增白名单 - 弹窗操作 */
  onAddPopup() {
    const modalRef = this.modalService.open(MessageWhiteListAddPopupComponent, {
      width: '744px',
      disableClose: true,
    });
