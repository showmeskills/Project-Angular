...
import { ReportLotteryListDetailComponent } from 'src/app/pages/finance/report/report/report-lottery/list/detail/detail.component';

@Component({
  templateUrl: './trade-lottery.component.html',
...
export class TradeLotteryComponent implements OnInit {
  category: any = 'Lottery'; // 类型
  tenantId: any = ''; // 商户ID
  uid: any = ''; // 会员UID
  ...
  async onExport() {
    ...
    const curCheckedArr = await Promise.all(
      this.list
        ...
        .map(async (e) => ({
          ...
          UID: e.uid,
          [info1]: e.lotterCategory,
          // [info2]: e.tournamentName || '',
          [game_id]: e.gameName,
          [trans_content]: e.roundNo + e.playWay,
          [currency]: e.currency,
    ...
    JSONToExcelDownload(curCheckedArr, 'report-lottery-list ' + Date.now());
  }
  ...
  async onView(item: any) {
    const res = await this.reportDetail.openDetail(
      this.api.getLotteryDetail(item.orderNumber),
      ReportLotteryListDetailComponent
    );
