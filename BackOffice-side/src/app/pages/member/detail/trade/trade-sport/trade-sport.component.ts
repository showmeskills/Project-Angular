...
import { ReportSportListDetailComponent } from 'src/app/pages/finance/report/report/report-sport/list/detail/detail.component';

@Component({
  templateUrl: './trade-sport.component.html',
...
export class TradeSportComponent implements OnInit {
  category: any = 'SportsBook'; // 类型
  tenantId: any = ''; // 商户ID
  uid: any = ''; // 会员UID
  ...
  async onExport() {
    ...
    const trans_content = await this.lang.getOne('game.provider.trans_content');
    ...
    const curCheckedArr = await Promise.all(
      this.list
        ...
        .map(async (e) => ({
          ...
          [game_id]: e.eventName,
          [trans_content]: e.betoptionName || e.betContent || '-',
          [currency]: e.currency,
    ...
    JSONToExcelDownload(curCheckedArr, 'report-sport-list ' + Date.now());
  }
  ...
  async onView(item: any) {
    const res = await this.reportDetail.openDetail(
      this.api.getSportDetail(item.orderNumber),
