...
import { ReportRealListDetailComponent } from 'src/app/pages/finance/report/report/report-real/list/detail/detail.component';

@Component({
  selector: 'trade',
  templateUrl: './trade-real.component.html',
...
export class TradeRealComponent implements OnInit {
  category: any = 'LiveCasino'; // 类型
  tenantId: any = ''; // 商户ID
  uid: any = ''; // 会员UID
  ...
  async onExport() {
    ...
    JSONToExcelDownload(curCheckedArr, 'report-real-list ' + Date.now());
  }
  ...
  async onView(item: any) {
    const res = await this.reportDetail.openDetail(
      this.api.getLivecasinoDetail(item.orderNumber),
      ReportRealListDetailComponent
    );
