...
import { ReportChessListDetailComponent } from 'src/app/pages/finance/report/report/report-chess/list/detail/detail.component';

@Component({
  templateUrl: './trade-chess.component.html',
...
export class TradeChessComponent implements OnInit {
  category: any = 'Chess'; // 类型
  tenantId: any = ''; // 商户ID
  uid: any = ''; // 会员UID
  constructor(
  ...
  async onExport() {
    ...
    JSONToExcelDownload(curCheckedArr, 'report-chess-list ' + Date.now());
  }
  ...
  async onView(item: any) {
    const res = await this.reportDetail.openDetail(
      this.api.getChessDetail(item.orderNumber),
