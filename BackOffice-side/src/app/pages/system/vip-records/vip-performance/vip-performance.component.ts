...
import { VipPerformanceItem } from 'src/app/shared/interfaces/vip-records.interface';
...
@Component({
  ...
  templateUrl: './vip-performance.component.html',
  styleUrls: ['./vip-performance.component.scss'],
  providers: [DestroyService],
})
export class VipPerformanceComponent implements OnInit {
  constructor(
  ...
  loadData(resetPage = false) {
    ...
    this.api
      .getVipPerformanceData(this.getParams())
  ...
  async onExport(isAll: boolean) {
    ...
    if (isAll) {
      ...
      try {
        this.appService.isContentLoadingSubject.next(true);
        const res = await lastValueFrom(this.api.getVipPerformanceData({ ...this.getParams(), pageSize: 9e5 }));
  ...
  async exportExcel(list: any[]) {
    ...
    const removals = await this.lang.getOne('report.playerItem.removals');
    ...
    JSONToExcelDownload(exportList, 'vip-performance' + Date.now());
  }
  ...
  onSort(sortColumn: string) {
    ...
    const multiplier = this.sortRow.isAsc ? -1 : 1;
