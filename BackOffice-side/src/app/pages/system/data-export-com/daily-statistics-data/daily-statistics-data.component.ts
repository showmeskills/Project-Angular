...
import { DailyStatisticsItem } from 'src/app/shared/interfaces/stat';
import { PageSizes, PaginatorState } from 'src/app/_metronic/shared/crud-table';
import { CurrencyValuePipe } from 'src/app/shared/pipes/currency-value.pipe';

@Component({
  selector: 'daily-statistics-data',
  standalone: true,
  imports: [
    CommonModule,
  ...
  templateUrl: './daily-statistics-data.component.html',
  styleUrls: ['./daily-statistics-data.component.scss'],
  providers: [DestroyService],
})
export class DailyStatisticsDateComponent implements OnInit {
  constructor(
  ...
  EMPTY_DATA = {
    // time: [moment().subtract(29, 'day').startOf('h').toDate(), new Date()] as Date[], // 时间范围
    time: [] as Date[], // 时间范围
    hasTest: false, // 是否包含测试用户
  };

  data = cloneDeep(this.EMPTY_DATA);
  list: DailyStatisticsItem[] = [];
  ...
  loadData(resetPage = false) {
    ...
    this.api
      .getDailyStatisticsData(this.getParams())
