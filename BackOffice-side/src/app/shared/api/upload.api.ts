...
export class UploadApi extends BaseApi {
  private _url = `${environment.apiUrl}/resource/upload`;
  ...
  /**
   * GoMoney获取上传接口
   * @param type PaymethodIcon
   * @param fileName 文件名包含后缀
   */
  createUploadURL(type: UploadType, fileName: string): Observable<UploadURL> {
    return this.post<any>(`${environment.apiUrl}/gomoney/system_createuploadurl`, {
      type,
      fileName: fileName ? encodeURI(fileName) : '',
    });
  }
  ...
  /**
   * 图片上传
   * @param file 要上传图片的file对象
   * @param type Merchant Currency GameProvider Games 图片上传类型
   * @param method
   */
  uploadForType(file: File, type: UploadType, method = 'gogaming'): Observable<any> {
    if (method == 'gogaming') {
      return this.getUploadURL(type, file.name, String(file.size)).pipe(
        switchMap((uploadURL) =>
          this.upload(uploadURL, file).pipe(map((res) => (res === null ? uploadURL.fullUrl : null)))
        )
      );
    } else {
      return this.createUploadURL(type, file.name).pipe(
        switchMap((uploadURL) =>
          this.upload(uploadURL, file).pipe(map((res) => (res === null ? uploadURL.filePath : null)))
        )
      );
    }
  }
