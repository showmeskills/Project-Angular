...
export class BaseApi {
   ...
   * @protected
   */
  protected get getAPMRequestID() {
    return MD5(`${Date.now()}${this.localStorageService.token}`).toString().slice(0, 16);
  }
  ...
  #contentThrowErrorFn: <T = any>(r: T, config: any) => T = (res) => res;

  /**
   * 设置内容过滤（返回false将会抛出错误 以及message）
   * @param fn
   * @protected
   * @desc 用于适配子服务接口不同返回格式
   */
  protected setContentFilter(fn: (res: any) => boolean) {
    this.#contentThrowErrorFn = (result, config) => {
      if (config?.skipContentFilter) return result;
      if (!fn(result)) {
        this.appService.showToastSubject.next({
          msg: result['message'] || result['error'] || result['msg'] || 'error',
        });
  ...
  get<T>(
    ...
    return this.http
      ...
      .pipe(
        ...
        map((e) => this.#contentThrowErrorFn(e, configOptions))
      );
  }
  ...
  delete<T>(
    ...
    return this.http.delete<T>(url, { ...configOptions }).pipe(
      catchError((error: any) => {
      ...
      this.langService
        ...
        .subscribe(async (fail) => {
          ...
          if (res.error instanceof Blob) {
            const error = await res.error.text();
            try {
