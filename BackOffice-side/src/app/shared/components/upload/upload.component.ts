 ...
 * 上传操作符
 */
export function uploadOperator<T = any>(
  initUploadValue?: Upload<T>
): (source: Observable<HttpEvent<T>>) => Observable<Upload<T>> {
  const initialState: Upload<T> = initUploadValue || {
    state: 'PENDING',
    progress: 0,
  };
  const calculateState = (upload: Upload<T>, event: HttpEvent<T>): Upload<T> => {
    if (!event) {
      return {
        ...upload,
        state: 'FAILED',
  ...
  return (source: Observable<HttpEvent<T>>) => source.pipe(scan(calculateState, initialState));
}
...
@Component({
  ...
  host: {
    '[class.is-disabled]': 'disabled || disabledInput',
    '[class.cursor-pointer]': '!(isDisabled && !value)',
  },
  standalone: true,
  imports: [
    NgTemplateOutlet,
...
export class UploadComponent implements OnInit, ControlValueAccessor {
   ...
   * 是否只读
   */
  @Input('readonly') set _readonly(value: string | boolean) {
  ...
  get isDisabled(): boolean {
    return this.disabledInput || this.disabled || this.readonly;
  }
  ...
  get isImg(): boolean {
    return [
      ...
      .map((e) => e.toLowerCase())
      .includes((this.filename.split('.').pop() || '').toLowerCase());
  }
   ...
   */
  get isVideo(): boolean {
    return ['mp4', 'ts', 'mov', 'mxf', 'mpg', 'flv', 'wmv', 'avi', 'm4v', 'f4v', 'mpeg', '3gp', 'asf', 'mkv']
      .map((e) => e.toLowerCase())
      .includes((this.filename.split('.').pop() || '').toLowerCase());
  }
   ...
   */
  get isPdf(): boolean {
    return ['pdf'].map((e) => e.toLowerCase()).includes((this.filename.split('.').pop() || '').toLowerCase());
  }
  ...
  async onChangeValue(fileDom: HTMLInputElement, value?: File) {
    ...
    if (value instanceof File) {
      ...
      // 上传格式错误，请上传 2.的格式
      const upfail = await this.lang.getOne('components.IncorrectUpload');
      const format = await this.lang.getOne('components.format');
      ...
      if (size && size < value.size) {
        // 如果设置限制kb大小提示
        // 请上传文件大小不超过
        const uploadSize = await this.lang.getOne('components.uploadSizeNolargerThan');
  ...
  uploadFile(imgFile: File) {
    ...
    const getUploadURL$ = this.api.getUploadURL(this.type, imgFile.name, mark);
  ...
  progress = (upload) => {
    ...
    if (upload.state === 'FAILED') {
      this.appService.showToastSubject.next({
        msgLang: 'common.uploadFailed', // 上传失败
        successed: false,
        duration: 5e3,
      });
    }
  ...
  onDrop(event: DragEvent) {
    ...
    if (files?.length) {
      this.onChangeValue(this.fileDom.nativeElement, files[0]);
    }
  }
}
