...
export class LangService {
   ...
   * @param key
   * @param params {LangParams}
   * ps: 主要可能未找到这个key时，返回undefined，提示需要处理，不然返回空字符串很难自然的发现
   */
   ...
   * @param params {LangParams}
   */
  public getOneByArr(key: LangKey[], params?: LangParams): Promise<string[]> {
    return Promise.all(key.map((k) => lastValueFrom(this.get(k, params).pipe(take(1)))));
  }
...
export class CurLangService implements OnDestroy {
  constructor(
    ...
    @Optional() @Inject(LangPrefix) private langPrefix: string
  ) {}
