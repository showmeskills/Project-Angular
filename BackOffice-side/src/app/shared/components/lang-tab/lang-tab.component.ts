...
// import { LanguageWarningComponent } from 'src/app/pages/Bonus/component/language-warning.component';
...
export class LangTabComponent implements OnInit, ControlValueAccessor, OnDestroy {
  ...
  @Input() suffixTpl!: TemplateRef<any>;
  ...
  onLanguageChange(languages: Language[]): void {
    ...
    if (hasRemove) {
      // TODO：放弃使用NgbActiveModal，个别情况会发生层级问题，导致无法正常显示，目前已调整为统一的提示弹窗组件
      // this.modal.open(LanguageWarningComponent, { centered: true }).result.then(({ value }) => {
      //   value && done();
      // });

      // 有删除 警告确认再赋值
      this.confirmModalService.open({ msgLang: 'components.langDelTip' }).result.then((value) => {
        value && done();
      });
    } else {
      done();
   ...
   * formArray语言
   */
  get formArrayValue(): FormArray {
    return this.formArray?.value?.length ? this.formArray : this.formArrayName?.control;
  }

  check() {
    const langArrayForm: any = this.formArrayValue;
    if ((!this.formArrayName && !this.formArray) || !langArrayForm) return;
