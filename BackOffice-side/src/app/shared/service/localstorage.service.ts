...
import { AESDecrypt, AESEncrypt } from 'src/app/shared/models/tools.model';
...
export class LocalStorageService {
  protected readonly SECRET_KEY_IP = 'Secret key ip';

  get ip() {
    const ip = LocalStorageService.getValue('ip') || '';
    return environment.isOnline ? AESDecrypt(ip, this.SECRET_KEY_IP) : ip;
  }

  set ip(ip: string) {
    ip = environment.isOnline ? AESEncrypt(ip, this.SECRET_KEY_IP) : ip;
    LocalStorageService.setValue('ip', ip);
  }
  ...
  get lang() {
    let cacheLang = LocalStorageService.getValue(LANGUAGE);

    // 空取浏览器语言
    if (!cacheLang) {
      cacheLang =
        (['zh-CN'].includes(window.navigator.language) && window.navigator.language) || // 浏览器语言
        (['en'].includes(window.navigator.language) && 'en-US') || // 英文浏览器语言
        'en-US';
      this.lang = cacheLang;
    }
