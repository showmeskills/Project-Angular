...
import {
  ...
  PickerType,
  SelectMode,
} from 'src/app/components/datetime-picker/lib/date-time/date-time.class';
import { OwlDialogRef } from 'src/app/components/datetime-picker/lib/dialog/dialog-ref.class';
import { OwlDialogService } from 'src/app/components/datetime-picker/lib/dialog/dialog.service';
...
export class OwlDateTimeComponent<T> extends OwlDateTime<T> implements OnInit, OnDestroy {
  ...
  get startAt(): any {
    ...
    if (this._dtInput) {
      if (this._dtInput.selectMode === 'single') {
        ...
        this._dtInput.selectMode === 'date-range' ||
  ...
  private dialogRef: any | OwlDialogRef<OwlDateTimeContainerComponent<T>>;
  ...
  public select(date: T[] | T): void {
    ...
    if (
      ...
      ((this.selectMode === 'single' && this.selected) ||
        ...
        ((this.selectMode === 'range' || this.selectMode === 'date-range') && this.selecteds[0] && this.selecteds[1]))
    ) {
      this.confirmSelect();
    }
  }
