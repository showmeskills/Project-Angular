...
part 'gaming_wallet_history_deposit_state.dart';

class GamingWalletHistoryDepositLogic extends BaseController
    with RefreshControllerMixin {
  final state = GamingWalletHistoryDepositState();
  ...
  LoadCallback get onLoadMore => (p) async {
        if (baseController.state.index != 0) return;
  ...
  void loadCryptoHistory() {
    ...
    getHistoryDepositCryptoList(state.cryptoDataIndex).listen((event) {
  ...
  Stream<GoGamingResponse<GoGamingPagination<GamingCryptoHistoryModel>>>
      getHistoryDepositCryptoList(int pageIndex) {
  ...
  void loadCurrencyHistory() {
    ...
    getHistoryDepositCurrencyList(state.currencyDataIndex).listen((event) {
  ...
  Stream<GoGamingResponse<GoGamingPagination<GamingCurrencyHistoryModel>>>
      getHistoryDepositCurrencyList(int pageIndex) {
    String currency = baseController.state.currency?.currency ?? '';
    if (currency == localized('all')) {
      currency = '';
    }
