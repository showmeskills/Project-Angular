...
enum GamingCurrencyDepositOrderStatus { handling, success, fail }

class CurrencyDepositResultConfirmLogic extends BaseController
    with GetTickerProviderStateMixin, DepositCommonUtilsMixin {
  final GamingCurrencyDepositModel data;

  CurrencyDepositResultConfirmLogic({required this.data});

  final _status = GamingCurrencyDepositOrderStatus.handling.obs;
  GamingCurrencyDepositOrderStatus get status => _status.value;

  String get statusText {
    switch (status) {
      case GamingCurrencyDepositOrderStatus.fail:
        return localized('time_expired');
      case GamingCurrencyDepositOrderStatus.success:
  ...
  void openWebView() {
    if (data.isHtmlOnlineBank) {
      ...
      if (data.redirectUrl != null) {
        ...
        //   },
        // );
        UrlSchemeUtil.openUrlWithBrowser(data.redirectUrl!);
      }
    }
  }

  void _onEnd() {
    if (status != GamingCurrencyDepositOrderStatus.handling) return;
    _status.value = GamingCurrencyDepositOrderStatus.fail;
  }
  ...
  void _refresh() {
    ...
    expireTime -= 1;
    if (expireTime <= 0 &&
        status != GamingCurrencyDepositOrderStatus.handling) {
      _status.value = GamingCurrencyDepositOrderStatus.fail;
      ...
      PGSpi(Deposit.orderStateInfo.toTarget(
        ...
        if (event.data?.state == 'Fail') {
          ...
          _status.value = GamingCurrencyDepositOrderStatus.success;
