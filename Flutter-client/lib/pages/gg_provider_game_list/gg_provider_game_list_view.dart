...
import 'package:gogaming_app/pages/gg_provider_game_list/gg_provider_game_list_state.dart';
import 'package:gogaming_app/pages/gg_provider_game_list/views/gaming_provider_label_selector_view.dart';
...
import 'gg_provider_game_list_logic.dart';

class GGProviderGameListPage extends BaseView<GGProviderGameListLogic> {
  const GGProviderGameListPage(
    this.providerId, {
    required this.type,
    super.key,
  });
  ...
  @override
  String? get tag => providerId;

  GGProviderGameListState get state => controller.state;
    ...
    return GetPage(
      ...
      page: () {
        ...
        return GGProviderGameListPage(
          providerId,
          type: type ?? GameSortType.provider,
        );
      },
    );
  }
  ...
  PreferredSizeWidget? appBar(BuildContext context) {
    return GGAppBar.userBottomAppbar(
      title: getProviderById(providerId)?.providerName ?? '',
      bottomHeight: 80.dp,
  ...
  Widget body(BuildContext context) {
    Get.put(GGProviderGameListLogic(type: type), tag: providerId);
  ...
  GamingGameProviderModel? getProviderById(String id) {
    return GameService.sharedInstance.provider.firstWhereOrNull((element) {
      return element.providerCatId == id;
    });
  }
}

extension _Action on GGProviderGameListPage {
  ...
  void _pressSelectProvider() {
    GamingBottomSheet.show<dynamic>(
      title: '${localized("sen_de")}${localized("type")}',
      builder: (context) {
        return GamingSelectProviderLabelSelector(
          providers: state.optionalProviders,
